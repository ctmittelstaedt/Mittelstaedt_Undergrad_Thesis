##################################################################################
# Extracting MFCCs from pika calls and conducting AP clustering on MB calls
...
input.dir <- file.path("data","individuality", "MB_individuals")
...
apres <- apcluster(negDistMat(r=2), x2, q=0.9, details=TRUE, maxits = 100000,
                   convits = 10000,
                   nonoise = T
                   )
...
# Show dendrograms
plot(aggres1)
dendro1 <-plot(aggres1, showSamples=TRUE)
